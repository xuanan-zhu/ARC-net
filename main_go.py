...
def parse_args():
    ...
    parser.add_argument('--go', default='BP')
...
if __name__ == "__main__":
    ...
    gearnet = ARC_net(input_dim=64, hidden_dims=[256, 256, 256, 256, 256, 256],
                                                        num_relation=2+21,
                                                        edge_input_dim=52+2+21, num_angle_bin=8,
    ...
    graph_construction_model = GraphConstruction(node_layers=[geometry.AlphaCarbonNode()],
                                                 edge_layers=[geometry.SpatialEdge(radius=10.0, min_distance=0),
                                                              geometry.KNNEdge(k=10, min_distance=0),
                                                              geometry.SequentialEdge(max_distance=10)],
                                                 edge_feature="gearnet")
    cfg = {'output_dir': '~/scratch/protein_output',
                      'dataset': {'class': 'GeneOntology', 'path': '~/scratch/protein-datasets/', 'branch':args.go, 'test_cutoff': 0.95,
